import {HandleResponse, Response, HandlerContext, Respondable, Plan, Message} from '@atomist/rug/operations/Handlers'
...
class GenericErrorHandler implements HandleResponse<any> {
    ...
    handle(response: Response<any>): Plan {
        ...
        return Plan.ofMessage(new Message(renderError(`${msg}${response.msg()}${body}`, this.corrid)));
    }
}
...
class GenericSuccessHandler implements HandleResponse<any> {
    ...
    handle(response: Response<any>): Plan {
        ...
        return Plan.ofMessage(new Message(renderSuccess(`${this.msg}`)));
    }
}
